      ...
      <section className="bg-slate-100 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { type: 'image', src: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=400&q=80', label: 'Clodomir' },
              { type: 'image', src: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=400&q=80', label: 'Amanda Airyes' },
              { type: 'image', src: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&q=80', label: 'Campos CRC' },
              { type: 'image', src: 'https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?w=400&q=80', label: 'Depoimento' },
            ].map((item, idx) => (
              <div key={idx} className="bg-white p-2 rounded-2xl shadow-md hover:shadow-xl transition-shadow overflow-hidden aspect-square relative group cursor-pointer" onClick={() => setZoomedMedia(item)}>
                <img src={item.src} alt="Depoimento" className="w-full h-full object-cover rounded-xl border border-slate-100" />
                <div className="absolute inset-2 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none rounded-xl">
                  <span className="text-white font-bold bg-slate-900/70 px-4 py-2 rounded-lg flex items-center gap-2 backdrop-blur-sm shadow-lg"><Search size={18}/> Ampliar</span>
                </div>
              </div>
            ))}
          </div>
