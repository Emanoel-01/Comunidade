...
import {
  ...
  Briefcase, Download, Calendar, Trash2, Save, AlertTriangle, LogOut, MessagesSquare, MessageSquare, ThumbsUp, ThumbsDown, BarChart2, Mail
} from 'lucide-react';
    ...
    <div className="bg-slate-100 min-h-screen flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0">
        ...
        <nav className="p-3 flex flex-col gap-1 flex-grow overflow-y-auto">
          ...
          <NavItem id="newsletter" label="Newsletter" icon={Mail} active={adminTab} onClick={setAdminTab} />
        ...
        <div className="bg-white rounded-2xl border border-indigo-200 shadow-md p-6 relative">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" disabled={saving} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-sm disabled:opacity-50 flex items-center gap-2"><Save size={14} />{saving ? 'Salvando...' : editingJob ? 'Salvar Alterações' : 'Publicar Vaga'}</button>
            </div>
          </form>
        </div>
      )}
