...
const tabs = [
  ...
  { id: 'forum', TabIcon: MessagesSquare, label: 'Fórum' },
        ...
        <div className="grid grid-cols-8 h-16">
          {tabs.map(({ id, TabIcon, label }) => (
            <button
              key={id}
              onClick={() => onTabChange(id)}
              className={`flex flex-col items-center justify-center gap-0.5 transition-colors ${activeTab === id ? 'text-indigo-600' : 'text-slate-400'}`}
            >
              <TabIcon size={20} strokeWidth={activeTab === id ? 2.5 : 1.8} />
