...
import { ChevronLeft, ChevronRight, Award, Briefcase, GraduationCap, Wifi } from 'lucide-react';

export default function TrajetoriaPortfolio() {
  ...
  const timeline = [
    ...
    { year: '2015-2017', title: 'Especialização em Gestão de Projetos e Obras', description: 'TCC: Diagnóstico dos Sistemas de Guarda Temporária de Lixo em Edificações Residenciais na Cidade do Recife/PE. Orientação: Profa. Msc. Renata Leça.', icon: 'academic' },
  ...
  const scrollTimeline = (direction) => {
    if (timelineRef.current) {
      timelineRef.current.scrollBy({ left: direction === 'left' ? -340 : 340, behavior: 'smooth' });
    }
  };
  ...
  const getIconConfig = (iconType) => {
    switch(iconType) {
      case 'tech': return { Icon: Wifi, color: 'text-indigo-600', bg: 'bg-indigo-100', border: 'border-indigo-200' };
      case 'academic': return { Icon: GraduationCap, color: 'text-blue-600', bg: 'bg-blue-100', border: 'border-blue-200' };
      case 'work': return { Icon: Briefcase, color: 'text-emerald-600', bg: 'bg-emerald-100', border: 'border-emerald-200' };
      case 'project': return { Icon: Award, color: 'text-orange-600', bg: 'bg-orange-100', border: 'border-orange-200' };
      default: return { Icon: Briefcase, color: 'text-slate-600', bg: 'bg-slate-100', border: 'border-slate-200' };
    }
  };
  ...
  return (
    <div className="bg-slate-50 py-16 px-4">
      <div className="w-full max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-6 px-2 sm:px-4">
          <div>
            <h2 className="text-3xl font-extrabold text-slate-900 flex items-center gap-3 tracking-tight">
              <Award className="w-8 h-8 text-indigo-600" />
              Trajetória & Portfólio
            </h2>
            <p className="text-slate-500 mt-2 font-medium">Deslize para ver o histórico profissional e acadêmico completo.</p>
          </div>
          <div className="flex items-center gap-3 bg-white p-1.5 rounded-xl shadow-sm border border-slate-200">
            <button onClick={() => scrollTimeline('left')} className="p-2.5 rounded-lg hover:bg-slate-100 text-slate-600 transition-colors" aria-label="Rolar para a esquerda">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button
              onClick={() => setIsAutoScroll(!isAutoScroll)}
              className={`px-5 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 ${isAutoScroll ? "bg-indigo-600 text-white shadow-md hover:bg-indigo-700" : "bg-slate-50 text-slate-700 hover:bg-slate-100"}`}
            >
              {isAutoScroll ? 'Pausar Reprodução' : 'Reprodução Automática'}
            </button>
            <button onClick={() => scrollTimeline('right')} className="p-2.5 rounded-lg hover:bg-slate-100 text-slate-600 transition-colors" aria-label="Rolar para a direita">
        ...
        >
          <div className="absolute top-0 left-0 w-8 sm:w-16 h-full bg-gradient-to-r from-slate-50 to-transparent z-10 pointer-events-none"></div>
          <div className="absolute top-0 right-0 w-8 sm:w-16 h-full bg-gradient-to-l from-slate-50 to-transparent z-10 pointer-events-none"></div>
          ...
          >
            {timeline.map((item, index) => {
              const { Icon, color, bg, border } = getIconConfig(item.icon);
              return (
                <div
                  key={index}
                  className="flex-shrink-0 w-[320px] h-[440px] rounded-3xl shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 relative group overflow-hidden bg-white snap-center border border-slate-200"
                >
                  {item.imageUrl ? (
                    <>
                      <div className="absolute top-5 left-5 z-20">
                        <div className="bg-white/20 backdrop-blur-md px-3.5 py-1.5 rounded-lg border border-white/30 shadow-sm">
                          <span className="text-xs font-black text-white tracking-widest uppercase">{item.year}</span>
                        </div>
                      </div>
                      <div className="absolute inset-0 w-full h-full bg-slate-900 z-0">
                        <img src={item.imageUrl} alt={item.title} className="w-full h-full object-cover opacity-90 group-hover:scale-105 transition-transform duration-700 ease-out" />
                      </div>
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-900/80 to-transparent z-10 opacity-90 group-hover:opacity-100 transition-opacity duration-300"></div>
                      <div className="absolute bottom-0 left-0 w-full p-6 z-20">
                        <h4 className="font-bold text-lg text-white mb-3 leading-snug">{item.title}</h4>
                        <div className="w-10 h-1 bg-indigo-500 mb-4 rounded-full opacity-90"></div>
                        <p className="text-sm text-slate-200 leading-relaxed font-light line-clamp-4 text-justify">{item.description}</p>
                      </div>
                    </>
                  ) : (
                    <div className="w-full h-full flex flex-col p-6 bg-gradient-to-br from-white to-slate-50 relative z-10">
                      <div className={`absolute top-0 right-0 w-32 h-32 ${bg} rounded-bl-full opacity-30 -mr-4 -mt-4 transition-transform group-hover:scale-110 duration-500 z-0`}></div>
                      <div className="flex justify-between items-start mb-8 relative z-10">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border ${bg} ${border}`}>
                          <Icon className={`w-7 h-7 ${color}`} />
                        </div>
                        <div className="bg-white px-3.5 py-1.5 rounded-lg border border-slate-200 shadow-sm">
                          <span className="text-xs font-black text-slate-700 tracking-widest uppercase">{item.year}</span>
                        </div>
                      </div>
                      <div className="relative z-10 flex-grow">
                        <h4 className="font-bold text-lg text-slate-900 mb-3 leading-snug">{item.title}</h4>
                        <div className={`w-10 h-1 rounded-full mb-4 opacity-80 ${bg.replace('100', '500')}`}></div>
                        <p className="text-sm text-slate-600 leading-relaxed text-justify">{item.description}</p>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <style dangerouslySetInnerHTML={{__html: `div[ref] { scrollbar-width: none; } div::-webkit-scrollbar { display: none; }`}} />
    </div>
  );
}