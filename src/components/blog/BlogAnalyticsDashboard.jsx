...
export default function BlogAnalyticsDashboard() {
  ...
  if (isLoading) return <div className="text-center py-8 text-gray-500">Carregando analytics...</div>;
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {[7, 30, 90].map(d => (
          <button key={d} onClick={() => setPeriodo(d)}
            className={`px-3 py-1 rounded-full text-sm font-medium transition-all ${periodo === d ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            {d} dias
          </button>
        ))}
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <BarChart2 className="w-5 h-5 text-pink-600" /> Posts Mais Vistos
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {topMidias.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Video className="w-5 h-5 text-red-500" /> Mídias Mais Acessadas
              </CardTitle>
            </CardHeader>
              ...
              {topMidias.map((m, i) => (
                <div key={i} className="flex items-center justify-between gap-2 py-2 border-b last:border-0">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-bold shrink-0 ${tipoColor(m.tipo)}`}>{m.tipo}</span>
        ...
        {topReferrers.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Globe className="w-5 h-5 text-blue-500" /> Origem dos Acessos
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {topReferrers.map(([referrer, count]) => (
                <div key={referrer} className="flex items-center justify-between py-2 border-b last:border-0">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-2 bg-pink-400 rounded-full" style={{ width: `${Math.max(20, (count / totalViews) * 100)}px` }} />
